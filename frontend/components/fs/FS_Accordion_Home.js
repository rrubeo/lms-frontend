...
export default function ControlledAccordions() {
    ...
    return (
        ...
        <Typography variant='h6' className={jnStyles.jnD1}>{accordionElements.title}</Typography>
                ...
                <Container disableGutters maxWidth='false' className={fsStyle.accordionContainer} key={item.id}>
                    <Accordion className={fsStyle.accordionElement} expanded={expanded === item.id} onChange={handleChange(item.id)}>
                        <AccordionSummary className={fsStyle.accordionSummary}
                          sx={{minHeight: '110px'}}
                          expandIcon={expanded === item.id?<Typography variant='h4' className='icon-arrow-down3' sx={{color: '#000000'}}></Typography>:<Typography variant='h4' className='icon-arrow-down3' sx={{color: '#000000'}}></Typography>}
                        >
                            ...
                            <Box className={fsStyle.accordionTitleContainer}>
                                <Stack direction='row'>
                                    <Box className={fsStyle.accordionTitleDivContainer}>
                                        <Typography variant='h6' className={jnStyles.jnD4}>Fisica fondamenti</Typography>
                                    </Box>   
                                    <Box className={fsStyle.accordionPercentageContainer}>
                                        <Typography variant='h6' className={jnStyles.jnD5}>30%</Typography>
                                    </Box>   
                                </Stack>
                                <Stack>
                        ...
                        <AccordionDetails className={fsStyle.accordionDetailContainer}>
                          {item.contentText.map(itemContent =>
                            <Container key={itemContent.id} disableGutters maxWidth='false' sx={{paddingBottom: '50px'}}>
                              <Typography variant='h6' className={jnStyles.jnD1}>{itemContent.arg}</Typography>
                              <List dense={true}>
                                {itemContent.list.map(itemObjectList =>
                                  <ListItem key={itemObjectList.id} sx={{padding: '0'}}>
                                    <Box sx={{width: '70%', borderBottom: '1px solid #ffffff', padding: '15px 0'}}>
                                      <Typography variant='p' className={jnStyles.jnI2}>{itemObjectList.title}</Typography>
                                    </Box>
                                    <Box sx={{width: '30%', textAlign: 'right'}}>
                                      <Typography variant='p' className={jnStyles.jnL2}>Stimato {itemObjectList.time} m</Typography>
                                    </Box>
                                  </ListItem>
                                )}
                              </List>
