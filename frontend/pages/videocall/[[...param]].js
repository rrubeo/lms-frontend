...
export const getServerSideProps = withIronSessionSsr(async function ({
  ...
  fallback.apiUrl = call_cfg.CALL_API + "?roomId=" + query.param[0];
...
function Main() {
  ...
  if (validRoomId) {
    activeRoom =
      new Date() >= new Date(data.appuntamento.appuDataInizioAppuntamento) &&
      new Date() <= new Date(data.appuntamento.appuDataFineAppuntamento);
  }
  if (activeRoom) {
    validUser =
      userInfo.login.toUpperCase() ===
        data.appuntamento.appuFkUtntRichiedente.toUpperCase() ||
      userInfo.login.toUpperCase() ===
        data.appuntamento.appuFkUtntRichiesta.toUpperCase();
  }
